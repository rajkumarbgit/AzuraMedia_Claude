...
export async function GET(req: NextRequest) {
  ...
  const users = await prisma.user.findMany({
    ...
    select: { id: true, name: true, email: true, role: true, defaultShift: true, designation: { select: { name: true } } },
    orderBy: { name: "asc" },
  });
...
export async function POST(req: NextRequest) {
  ...
  const { name, email, password, role, designationId, defaultShift } = body;
